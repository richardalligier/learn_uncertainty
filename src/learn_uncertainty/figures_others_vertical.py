...
def draw_others(sit,json,device,uparam,uncertainty_value,all_beacons=True,rotated =False,fname=None):
    ...
    #     print(fid,mindisti)
    xy_u = dxy_u["others"]
    tz_u = add.compute_tz(uparams)["others"]
    xy_u = tz_u
    print(xy_u.names)
    ...
    points = {k:apply_mask(sit["others"].generate_tz(ti),sit["others"].generate_mask(ti,thresh=20)) for k,ti in t.items()}
    # print(sit["others"].fz)
    # raise Exception
...
#    print(sit["deviated"].tzero+sit["deviated"].tdeviation)
    ...
    draw_others(sit,json,device,"vspeed",torch.tensor([0.8,1.2],device=device).rename(VALUESTOTEST),all_beacons=False,rotated=False,fname="./figures/vspeed.pdf")
